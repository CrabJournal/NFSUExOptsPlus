...
bool ShowMoreRaceOptions, ShowLanguageSelectScreen, AnyTrackInAnyMode, MorePaintTypes, ShowSpecialVinyls, ExOptsTeamTakeOver, mpsInsteadOfMiPH;
bool EnableHiddenCameras, UnlockAllThings, ShowTournament, GarageZoom, GarageRotate, ShowDebugOptionsMenu, SkipMovies, EnableSound, EnableMusic;
bool AllowMultipleInstances, once1, once2, WheelFix, DamageFix, EnablePreRaceStats, NoGravity, NoOutOfMapReset, DriftPhysicsInAnyMode;

float SplashScreenTimeLimit, GameSpeed, MiniWorldCarSpeedCruising, MiniWorldCarSpeedFast, MiniWorldCarSpeedSlow, JumpSpeed, DragCrashSensitivity;
...
void ReadConfig()
{
	CIniReader iniReader("NFSUExtraOptions+Settings.ini");
	...
	// Gameplay
	DriftPhysicsInAnyMode = iniReader.ReadInteger("Gameplay", "DriftPhysicsInAnyMode", 0) != 0;
	DragCrashSensitivity = iniReader.ReadFloat("Gameplay", "DragCrashSensitivity", 35.0f);
	DragInAnyMode = iniReader.ReadInteger("Gameplay", "DragInAnyMode", 0);
	JumpSpeed = iniReader.ReadFloat("Gameplay", "JumpSpeed", 20.0f);
	NoOutOfMapReset = iniReader.ReadInteger("Gameplay", "NoOutOfMapReset", 0) != 0;
	...
	// Misc
	mpsInsteadOfMiPH = iniReader.ReadInteger("Misc", "mpsInsteadOfMiPH", 0) != 0;
...
void Init()
{
	...
	if (NoOutOfMapReset)
	{
		injector::MakeNOP(0x42653E, 5, true);
	}
	...
	if (DragCrashSensitivity > 50.0)
		injector::WriteMemory<float>(0x6B72C0 /* .rdata */, DragCrashSensitivity, true);
	injector::WriteMemory<float>(0x6B72C8 /* .rdata */, DragCrashSensitivity, true);

	switch (DragInAnyMode) {
	case 2:
		// setting laps number
		injector::WriteMemory<BYTE>(0x4342CF, 0xEB /*jmp short*/, true);
		// control mode
		injector::MakeNOP(0x430ADE, 2, true);
		// AI control mode
		injector::MakeNOP(0x425184, 2, true);
	...
	case 3:
		// blow engine
		injector::MakeNOP(0x46CAF2, 6, true);
		// crash
		injector::MakeNOP(0x465DAE, 6, true);
		// drag's end of race
		injector::MakeNOP(0x43578C, 6, true);
		// drag's post race sign
		injector::MakeNOP(0x4A7E51, 6, true);
		break;
	default:
		break;
	}
